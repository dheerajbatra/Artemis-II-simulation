...
from physics import (
    propagate, moon_position, MU_EARTH, R_EARTH, R_MOON,
    closest_approach, reentry_index, MOON_SEMI_MAJOR, MOON_PERIOD
)
...
def tli_state(dec_deg=TLI_DEC_DEG, v_mag=V_TLI_DEFAULT):
    """
    ...
    return np.array([PARKING_R, 0.0, 0.0, 0.0,
                     v_mag * np.cos(dec),
                     v_mag * np.sin(dec)])
...
def score_trajectory(params):
    ...
    try:
        t, states = propagate(state0, (0, 10*86400),
                              moon_phase0=phase0, n_points=15000,
                              include_sun=False, rtol=1e-9, atol=1e-9)
...
def optimize_free_return(verbose=True):
    ...
    result = minimize(score_trajectory, x0, method='Nelder-Mead',
                      options={'xatol': 0.005, 'fatol': 1.0,
                               'maxiter': 200, 'disp': verbose})
    dec, phase_deg = result.x
    if verbose:
        print(f"  Optimized: dec={dec:.3f}°  phase={phase_deg:.3f}°  score={result.fun:.2f} km")
...
def build_full_trajectory(dec=None, phase0_deg=None, optimize=True):
    ...
    if optimize:
        ...
        if dec        is None: dec        = TLI_DEC_DEG
        if phase0_deg is None: phase0_deg = MOON_PHASE0_DEG
    ...
    t, states = propagate(state0, (0, MISSION_DURATION),
                          moon_phase0=phase0, n_points=100000,
    ...
    if idx_re:
        print(f"  Re-entry at T+     : {t[idx_re]/3600:.2f} h  ({t[idx_re]/86400:.3f} days)")
